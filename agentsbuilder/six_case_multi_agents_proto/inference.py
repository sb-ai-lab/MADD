...
def bot(history: list[list]) -> list[list]:
    ...
    try:
        ...
        onlyfiles = [
            f
            for f in listdir("agentsbuilder/six_case_multi_agents_proto/vizualization")
            if isfile(join("agentsbuilder/six_case_multi_agents_proto/vizualization", f))
        ]
    except:
        answer = chain.run(history[-1][0])
        onlyfiles = [
    ...
    try:
        if onlyfiles != []:
            ...
            for file in sorted_files[::-1]:
                image = gr.Image("agentsbuilder/six_case_multi_agents_proto/vizualization/" + file)
    ...
    # if answer without molecules
        ...
        if onlyfiles != []:
            for file in onlyfiles:
                image = gr.Image("agentsbuilder/six_case_multi_agents_protovizualization/" + file)
