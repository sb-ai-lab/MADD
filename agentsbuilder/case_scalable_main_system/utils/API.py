...
with open("agentsbuilder/case_scalable_main_system/config.yaml", "r") as file:
...
def call_for_train_model(model: str, epoch: int, case_name: str, property: str = None):
    ...
    """
    if model == 'AUTOML':
        url = f"http://{models_ip_addr}/automl"
        params = {"property": property}
    else:
        url = f"http://{models_ip_addr}/train_gen_model"
...
def call_for_docking(
    mol_list: Union[requests.models.Response, List[str], dict],
    url: str = f"http://{models_ip_addr}/docking",
) -> Tuple[requests.models.Response, dict]:
