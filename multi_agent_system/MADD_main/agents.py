...
from prompting.chat_prompts import (
    HELP_FOR_TOOLS,
...
from prompting.validator_prompts import (
    query_pattern_valid,
...
from multi_agent_system.MADD_main.tools.data_gathering import (
...
if config["prompt_type"] == 1:
    from prompting.conductor_prompts_v1 import (
    ...
    from prompting.conductor_prompts_r1 import (
        INSTRUCT_TOOLS_LARGE,
        INSTRUCT_TOOLS_LITTLE,
        prompt_few_funcs,
        prompt_many_funcs,
    ...
    from prompting.conductor_prompts_v2 import (
        INSTRUCT_TOOLS_LARGE,
        INSTRUCT_TOOLS_LITTLE,
        prompt_few_funcs,
        prompt_many_funcs,
...
def automl_agent(state: dict, config: dict) -> Command:
    ...
    if dataset != ["False"]:
        ...
        task_formatted = (
            f"""\nYou are tasked with executing: {task}. Attention: Check if there is a case on the server, if there is one, do not start the training. If there no such case, you MUST use 'run_ml_dl_training_by_daemon'!!! Columns in users dataset: """
            + str(list(dataset_columns))
            + f"You must pass target_column one of these column names. Feature column should be it must be something related to the SMILES string (find the correct name). Pass this path ({dataset[0]}) to 'path'! YOU MUST ALWAYS CALL TOOLS!"
        )
    else:
        agent = create_react_agent(
            config["configurable"]["llm"],
